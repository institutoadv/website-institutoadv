...
const App = () => {
  ...
  const projectDetails = {
    informatica: { title: "Informática Básica", tag: "Inclusão Digital", icon: <Laptop size={48} />, color: "blue", image: "https://images.unsplash.com/photo-1531297484001-80022131f5a1?auto=format&fit=crop&q=80&w=1000", description: "A exclusão digital é um isolamento social.", longText: "Preparamos o aluno para o mercado moderno, desde o hardware básico até ferramentas de produtividade e LinkedIn.", benefits: ["Windows/Linux", "Pacote Office", "Criação de Currículo", "LinkedIn", "Internet Segura"], info: { duration: "3 meses", hours: "60h", target: "Jovens e Adultos" } },
    eja: { title: "Alfabetização (EJA)", tag: "Dignidade e Resgate", icon: <GraduationCap size={48} />, color: "green", image: "https://images.unsplash.com/photo-1456735190827-d1262f71b8a3?auto=format&fit=crop&q=80&w=1000", description: "Nunca é tarde para descobrir o mundo através das letras.", longText: "Alfabetizar um adulto é devolver-lhe a voz na sociedade. Focamos na leitura funcional e no orgulho de assinar o nome.", benefits: ["Método Afetivo", "Matemática do Quotidiano", "Interpretacão de Textos", "Cidadania", "Apoio Encceja"], info: { duration: "Contínuo", hours: "4h/sem", target: "Adultos e Idosos" } },
    infantil: { title: "Educação Infantil", tag: "Futuro e Proteção", icon: <Sparkles size={48} />, color: "yellow", image: "https://images.unsplash.com/photo-1587654780291-39c9404d746b?auto=format&fit=crop&q=80&w=1000", description: "A infância é o solo onde plantamos o amanhã.", longText: "Através de artes, música e reforço pedagógico lúdico, garantimos um ambiente de paz e aprendizado alegre.", benefits: ["Reforço Criativo", "Oficinas de Arte", "Música", "Horta Educativa", "Apoio Familiar"], info: { duration: "Anual", hours: "Seg-Sex", target: "Crianças 6-12" } }
  };
  ...
  const testimonials = [
    { name: "Dona Maria, 62 anos", text: "Hoje escrevo cartas para os meus netos. Mudou a minha vida!", role: "Aluna EJA", image: "https://images.unsplash.com/photo-1544120190-27583f22744c?auto=format&fit=crop&q=80&w=200" }
  ];
  ...
  const calculateAttendancePercentage = (attendanceArr) => {
    ...
    return Math.round((attendanceArr.filter(v => v === true).length / 10) * 100);
  };
  ...
  const ContactFormComponent = () => {
    ...
    if (status === 'success') return <div className="py-12 text-center text-green-500 font-bold">Mensagem Enviada!</div>;
    return (
      <form onSubmit={handleSend} className="space-y-5">
        <input required value={contactData.name} onChange={e => setContactData({ ...contactData, name: e.target.value })} placeholder="Seu Nome" className="w-full px-6 py-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-orange-500 shadow-inner font-bold" />
        <input required type="email" value={contactData.email} onChange={e => setContactData({ ...contactData, email: e.target.value })} placeholder="E-mail" className="w-full px-6 py-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-orange-500 shadow-inner font-bold" />
        <textarea required value={contactData.message} onChange={e => setContactData({ ...contactData, message: e.target.value })} placeholder="Como podemos ajudar?" className="w-full px-6 py-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-orange-500 shadow-inner font-bold" rows="4" />
        <button type="submit" className="w-full py-5 bg-orange-500 text-white font-black rounded-2xl uppercase shadow-lg">Enviar Mensagem</button>
      </form>
    );
  };
    ...
    if (status === 'success') return (
      <div className="pt-40 flex items-center justify-center min-h-screen bg-slate-50 p-4 font-sans">
        <div className="bg-white max-w-2xl w-full p-16 rounded-[4rem] shadow-2xl text-center border">
          ...
          <h2 className="text-4xl font-black italic mb-4">Matrícula Realizada!</h2>
          <div className="bg-slate-50 p-8 rounded-3xl mb-8 shadow-inner text-center">
            <p className="text-xs uppercase text-slate-400 font-black">Protocolo Azure</p>
            <p className="text-3xl font-black text-slate-800">#{lastProtocol}</p>
          </div>
          <button onClick={() => navigateTo('home')} className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black uppercase shadow-xl">Voltar ao Início</button>
        </div>
      </div>
    );
    ...
    return (
      <div className="pt-32 pb-20 px-4 bg-slate-50 min-h-screen">
        <div className="max-w-5xl mx-auto">
          <button onClick={() => navigateTo('home')} className="mb-8 font-bold text-slate-400 flex items-center gap-2 uppercase text-[10px] tracking-widest"><ArrowLeft size={16} /> Voltar</button>
          <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden border">
            <div className="bg-slate-900 p-10 text-white"><h2 className="text-3xl font-black italic">Formulário de <span className="text-orange-500">Matrícula</span></h2></div>
            <form onSubmit={handleSubmit} className="p-10 space-y-8">
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="md:col-span-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-2">Nome Completo</label><input required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full p-4 bg-slate-50 rounded-xl shadow-inner font-bold" /></div>
                <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2">Nascimento</label><input required type="date" value={formData.birthDate} onChange={e => setFormData({ ...formData, birthDate: e.target.value })} className="w-full p-4 bg-slate-50 rounded-xl shadow-inner font-bold" /></div>
                <div><label className="text-[10px] font-black uppercase text-slate-400 ml-2">CPF</label><input required value={formData.cpf} onChange={e => setFormData({ ...formData, cpf: e.target.value })} className="w-full p-4 bg-slate-50 rounded-xl shadow-inner font-bold" /></div>
              </div>
              <div className="grid md:grid-cols-4 gap-6">
                <input required placeholder="RG" value={formData.rg} onChange={e => setFormData({ ...formData, rg: e.target.value })} className="w-full p-4 bg-slate-50 rounded-xl shadow-inner font-bold" />
                <input required placeholder="WhatsApp" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} className="w-full p-4 bg-slate-50 rounded-xl shadow-inner font-bold" />
                <div className="md:col-span-2"><input required placeholder="Email" type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="w-full p-4 bg-slate-50 rounded-xl shadow-inner font-bold" /></div>
              </div>
              <div className="p-6 bg-slate-900 rounded-3xl text-white flex gap-4 shadow-xl">
                <input required type="checkbox" checked={formData.lgpdConsent} onChange={e => setFormData({ ...formData, lgpdConsent: e.target.checked })} className="w-6 h-6 rounded text-orange-500 shadow-inner" />
                <p className="text-xs italic">Concordo com os termos da LGPD no Instituto ADV.</p>
              </div>
              <button disabled={status === 'sending'} className="w-full py-6 bg-orange-500 text-white font-black rounded-3xl shadow-xl uppercase italic tracking-widest">{status === 'sending' ? 'Enviando...' : 'Finalizar Matrícula'}</button>
            </form>
          </div>
        </div>
      </div>
      ...
      <div className="pt-32 pb-20 px-4 min-h-screen bg-slate-50 font-sans">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-12">
            <h1 className="text-4xl font-black italic tracking-tighter">Gestão <span className="text-blue-600">Cloud</span></h1>
            <div className="flex gap-2 bg-white p-2 rounded-2xl shadow-sm border">
              <button onClick={() => setDashboardTab('alunos')} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${dashboardTab === 'alunos' ? 'bg-slate-900 text-white' : 'text-slate-400'}`}>Alunos</button>
              <button onClick={() => setDashboardTab('frequencia')} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${dashboardTab === 'frequencia' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}>Frequência</button>
              <button onClick={() => setDashboardTab('contatos')} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${dashboardTab === 'contatos' ? 'bg-orange-500 text-white' : 'text-slate-400'}`}>Contatos</button>
            </div>
          </div>

          <div className="bg-white p-4 rounded-3xl mb-8 flex gap-4 border shadow-sm items-center transition-all focus-within:shadow-md">
            <Search className="text-slate-300 ml-2" />
            <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Pesquisar..." className="w-full outline-none font-bold text-sm" />
          </div>

          <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden border">
            {dashboardTab === 'alunos' && (
              <table className="w-full text-left">
                <thead className="bg-slate-900 text-white text-[10px] font-black uppercase italic"><tr><th className="px-8 py-6">Protocolo</th><th className="px-8 py-6">Aluno</th><th className="px-8 py-6 text-center">Projeto</th><th className="px-8 py-6 text-center">Ações</th></tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {filteredRegs.map(reg => (
                    <tr key={reg.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-8 py-6 text-center uppercase text-[10px] font-black">{reg.course}</td>
            ...
            {dashboardTab === 'frequencia' && (
              <table className="w-full text-left">
                <thead className="bg-slate-900 text-white text-[10px] font-black uppercase italic"><tr><th className="px-8 py-6">Aluno</th><th className="px-8 py-6 text-center">% Freq</th><th className="px-8 py-6 text-center">Certificado</th></tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {filteredRegs.map(reg => {
                    ...
                    return (
                      <tr key={reg.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-8 py-6 text-center">
                          {perc >= 90 ? (
                            <button onClick={() => setSelectedCertificate(reg)} className="p-2 bg-orange-100 text-orange-600 rounded-lg hover:bg-orange-600 hover:text-white transition-all shadow-sm"><Award size={18} /></button>
                          ) : (
            ...
            {dashboardTab === 'contatos' && (
              <table className="w-full text-left">
                <thead className="bg-slate-900 text-white text-[10px] font-black uppercase italic"><tr><th className="px-8 py-6">Nome</th><th className="px-8 py-6">Assunto</th><th className="px-8 py-6">Mensagem</th></tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {filteredMsgs.map(msg => (
                    <tr key={msg.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-8 py-6 font-bold uppercase">{msg.name}</td>
                      <td className="px-8 py-6 text-xs text-orange-500 font-black uppercase">{msg.subject}</td>
                      <td className="px-8 py-6 text-sm text-slate-500 italic">"{msg.message}"</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl relative z-10 overflow-hidden font-sans animate-in zoom-in">
              ...
              <div className="p-10 space-y-6">
                <div className="grid grid-cols-2 gap-8">
                  <div><p className="text-[10px] uppercase font-black text-slate-400 mb-1">Nome Completo</p><p className="font-black uppercase text-lg text-slate-800">{selectedStudent.name}</p></div>
                  <div><p className="text-[10px] uppercase font-black text-slate-400 mb-1">CPF</p><p className="font-bold text-slate-700">{selectedStudent.cpf}</p></div>
                </div>
                <div className="p-6 bg-slate-50 rounded-2xl shadow-inner border">
                  <p className="text-[10px] uppercase font-black text-slate-400 mb-2">Endereço Registrado</p>
                  <p className="text-sm font-bold text-slate-600">{selectedStudent.logradouro}, {selectedStudent.numero} - {selectedStudent.bairro}</p>
                </div>
                <div className="flex items-center gap-3 p-4 bg-green-50 text-green-700 rounded-xl text-[10px] font-black uppercase tracking-widest"><ShieldCheck size={20} /> Dados Sincronizados Azure SQL</div>
              </div>
    ...
    <div className="pt-20 animate-in fade-in duration-700 font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 py-16 grid lg:grid-cols-3 gap-16">
        <div className="lg:col-span-2 space-y-12">
          <section><h2 className="text-4xl font-black mb-8 italic text-slate-800 underline decoration-orange-500 decoration-8 underline-offset-8">Sobre o Projecto</h2><p className="text-xl text-slate-600 leading-relaxed mb-8 font-medium italic">"{data.description}"</p><p className="text-lg text-slate-500 leading-relaxed">{data.longText}</p></section>
  ...
  // --- RENDERING PRINCIPAL ---
  if (loading) return <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white font-black italic gap-4 animate-pulse"><div className="w-16 h-16 border-4 border-orange-500 border-t-transparent rounded-full animate-spin"></div>CARREGANDO INSTITUTO ADV...</div>;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 overflow-x-hidden font-sans">
      ...
      <nav className="fixed w-full bg-white/90 backdrop-blur-md z-50 h-20 flex justify-between items-center px-4 lg:px-20 border-b border-slate-100 shadow-sm">
        <div className="flex items-center gap-2 cursor-pointer transition-transform hover:scale-105" onClick={() => navigateTo('home')}>
          ...
          <span className="text-xl font-black italic tracking-tighter">Instituto <span className="text-orange-500">Alegria de Viver</span></span>
        </div>
