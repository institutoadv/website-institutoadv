...
const App = () => {
    ...
    const loadFromSQL = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Normalização para evitar erros de Case Sensitivity (SQL vs JS)
  ...
  const calculateAttendancePercentage = (attendanceArr) => {
    ...
    const count = attendanceArr.filter(v => v === true || v === 1).length;
  ...
  const ContactFormComponent = () => {
    ...
    if (status === 'success') return <div className="py-12 text-center text-green-500 font-bold">Mensagem Enviada ao Banco!</div>;
    return (
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden border">
            ...
            <form onSubmit={handleSubmit} className="p-8 lg:p-12 space-y-10">
              {/* DADOS PESSOAIS COMPLETOS RESTAURADOS */}
              ...
              <div className="p-6 bg-slate-900 rounded-3xl text-white flex gap-4 shadow-xl">
                ...
                <p className="text-xs italic font-medium">Concordo com os termos da LGPD no Instituto ADV.</p>
              </div>
              <button disabled={status === 'sending'} className="w-full py-6 bg-orange-500 text-white font-black rounded-3xl shadow-xl uppercase italic tracking-widest hover:bg-orange-600 transition-all">{status === 'sending' ? 'Processando Cloud...' : 'Finalizar Matrícula'}</button>
            </form>
          </div>
        </div>
      </div>
      ...
      <div className="pt-32 pb-20 px-4 min-h-screen bg-slate-50 font-sans">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white p-4 rounded-3xl mb-8 flex gap-4 border shadow-sm items-center transition-all focus-within:shadow-md">
            ...
            <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Pesquisar registros no Azure..." className="w-full outline-none font-bold text-sm" />
          </div>
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden border border-slate-100">
            ...
            {dashboardTab === 'frequencia' && (
              <table className="w-full text-left">
                <thead className="bg-slate-900 text-white text-[10px] font-black uppercase italic tracking-widest"><tr><th className="px-8 py-6">Aluno</th><th className="px-8 py-6 text-center">% Presença</th><th className="px-8 py-6 text-center">Opções</th></tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {filteredRegs.map(reg => {
            ...
            {dashboardTab === 'contatos' && (
              <table className="w-full text-left">
                <thead className="bg-slate-900 text-white text-[10px] font-black uppercase italic tracking-widest"><tr><th className="px-8 py-6">Nome</th><th className="px-8 py-6">Assunto</th><th className="px-8 py-6">Mensagem</th></tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {filteredMsgs.map(msg => (
                    <tr key={msg.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-8 py-6 text-sm text-slate-500 italic max-w-sm truncate">"{msg.message}"</td>
                    </tr>
                  ))}
